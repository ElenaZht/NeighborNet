...
import { editHelpRequest } from '../../../features/reports/helpRequests/editHelpRequestThunk';
import AddressInputForm from '../../AddressInputForm'
import { categories } from '../../../utils/requestCategories';
import { EditHelpRequestFormProps, HelpRequestFormData, HelpRequestChangedFields } from './types';

export default function EditHelpRequestForm({ reportData, onSuccess, onError }: EditHelpRequestFormProps) {
  ...
  const [originalData, setOriginalData] = useState<HelpRequestFormData>({
    title: '',
    description: '',
    address: '',
    img_url: '',
  ...
  const handleAddressChange = (addressResult: { address: string; city: string; location: { lat: number; lng: number } }) => {
    setFormData(prev => ({
      ...prev,
  ...
  const getChangedFields = (): HelpRequestChangedFields => {
    const changes: HelpRequestChangedFields = {};
    
    Object.keys(formData).forEach(key => {
      if (key === 'location') {
        // Special handling for location object
        const originalLoc = originalData.location || { lat: '', lng: '' };
        const currentLoc = formData.location || { lat: '', lng: '' };
        ...
        const formValue = formData[key as keyof HelpRequestFormData];
        const originalValue = originalData[key as keyof HelpRequestFormData];
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const errorMessage = (error as Error).message || 'Failed to update help request';
