...
import { editIssueReport } from '../../../features/reports/issueReports/editIssueRequestThunk';
import AddressInputForm from '../../AddressInputForm'
import { EditIssueReportFormProps, IssueReportFormData, EditIssueReportData, AddressResult } from './types';

export default function EditIssueReportForm({ reportData, onSuccess, onError }: EditIssueReportFormProps) {
  const dispatch = useAppDispatch();
  const addressInputRef = useRef<{ clearAddress: () => void }>(null);
  
  const [formData, setFormData] = useState<IssueReportFormData>({
    title: '',
    description: '',
    address: '',
    img_url: '',
  ...
  const [originalData, setOriginalData] = useState<IssueReportFormData>({
    title: '',
    description: '',
    address: '',
    img_url: '',
  ...
  const handleAddressChange = (addressResult: AddressResult) => {
    setFormData(prev => ({
      ...prev,
  ...
  const getChangedFields = (): EditIssueReportData => {
    const changes: EditIssueReportData = {};
    ...
    Object.keys(formData).forEach(key => {
      if (key === 'location') {
        ...
        const formValue = formData[key as keyof IssueReportFormData];
        const originalValue = originalData[key as keyof IssueReportFormData];
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to update issue report';
